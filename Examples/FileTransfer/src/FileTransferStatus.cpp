...
FileTransferStatus::FileTransferStatus(uint32_t file_size, const std::string file_name, const UUID &receiver_transfer_id, const UUID &sender_transfer_id, bool mem_alloc) :
	file_size(file_size), file_name(file_name), receiver_transfer_id(receiver_transfer_id),
	sender_transfer_id(sender_transfer_id), transfered_size(0), start_time(::time(NULL)),
...
void FileTransferStatus::setReceiverTransferID(const UUID &transfer_id) {
	Poco::FastMutex::ScopedLock l(this->mutex);
	this->receiver_transfer_id = transfer_id;
}

void FileTransferStatus::setSenderTransferID(const UUID &transfer_id) {
	Poco::FastMutex::ScopedLock l(this->mutex);
...
bool FileTransferStatus::update(int data_number, const unsigned char *data, size_t len) {
	Poco::FastMutex::ScopedLock l(this->mutex);
